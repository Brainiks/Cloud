...
async function loadFiles() {
    try {
        const response = await fetch('http://0.0.0.0:8000/files/');
        if (response.ok) {
            const data = await response.json();
            ...
            if (data.files.length === 0) {
                ...
                data.files.forEach(file => {
                    ...
                    if (isImage) {
                        ...
                        fileItem.innerHTML += `
                            <img src="http://0.0.0.0:8000/file/${file.filename}" alt="${file.filename}">
                            <p>${file.filename}</p>
                        `;
                    } else {
...
async function downloadSelectedFiles() {
    ...
    for (const checkbox of checkboxes) {
        ...
        link.href = `http://0.0.0.0:8000/file/${filename}`;
